<?php
class Add_company_photo extends CI_controller
{
  ...
  { 
    $data['fetch_category']=$this->productmodel->fetch_company_image();
    $data['list']=$this->productmodel->fetch_product();
    $this->load->view('admin/template/header');
    $this->load->view('admin/template/sidebar');
    $this->load->view('admin/template/topbar');
    $this->load->view('admin/product/add_company_photo',$data);
  ...
  public function add(){
    ...
    $img_data = array();
            ...
            for($j=0; $j<$countfiles; $j++){
                ...
                $fina = $this->productmodel->upload_comapany_image($data);
    
            }

            if ($fina == true) {
                $this->session->set_flashdata('success', 'Added successfully');
                redirect(base_url() . 'admin/product/add_company_photo');
            } else {
  ...
  public function deletecontactdetail()
  {
    ...
    if ($this->input->post('deletesliderId')) {
      ...
      if ($this->form_validation->run() == true) {
        $getDeleteStatus = $this->productmodel->delete_company_image($this->input->post('deletesliderId'));
